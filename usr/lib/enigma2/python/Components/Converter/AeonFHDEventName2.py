...
#
#-----------------------------------------------------------------------------
# Jan.2017 (g55) small mods
#       - Primetime : delete spaces between times
#       - max items increased to 12
# Mar.2017 (g55) new PRIME_TIME_WITH_DURATION2 : inserts new line after duration
#-----------------------------------------------------------------------------
...
class AeonFHDEventName2(Converter, object):
	NAME = 0
	NAME_TWEAKED = 1
	SHORT_DESCRIPTION = 2
	...
	def __init__(self, type):
		...
		elif type == "PrimeTimeWithDuration2":
			self.type = self.PRIME_TIME_WITH_DURATION2
		else:
			self.type = self.NAME
	...
	def getText(self):
		...
		if self.type is self.NAME:
		...
		elif self.type is self.PRIME_TIME_NO_DURATION or self.type is self.PRIME_TIME_ONLY_DURATION or \
				self.type is self.PRIME_TIME_WITH_DURATION or self.type is self.PRIME_TIME_WITH_DURATION2 :
			...
			if current_event:
				...
				if next and (next.getBeginTime() <= int(mktime(dt.timetuple()))):
					...
					elif self.type is self.PRIME_TIME_WITH_DURATION2:
						return '%s-%s (%s)\n %s' % (begin, end, duration, title)
